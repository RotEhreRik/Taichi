...
init_pos = ti.Vector.field(2, dtype=ti.f64, shape=N)  # <<< AJOUT
init_vel = ti.Vector.field(2, dtype=ti.f64, shape=N)  # <<< AJOUT
...
for i in range(N):
    ...
    init_pos[i] = ti.Vector([init_pos_np[i, 0], init_pos_np[i, 1]])  # <<< AJOUT
    init_vel[i] = ti.Vector([init_vel_np[i, 0], init_vel_np[i, 1]])  # <<< AJOUT
...
def initialize():
    for method, i in ti.ndrange(METHODS, N):
        pos[method, i] = init_pos[i]  # <<< MODIFIÉ
        vel[method, i] = init_vel[i]  # <<< MODIFIÉ
...
os.makedirs("output", exist_ok=True)  # <<< AJOUT
