...
# CHORD_INTERVALS = [0, 3, 7, 10] #Mineur 7 :
# CHORD_INTERVALS = [0, 2, 7] #Sus2 / ouvert
# CHORD_INTERVALS = [0, 4, 7, 14] #Add9 large
CHORD_INTERVALS = [0, 2, 4, 7, 9] #Pentatonique majeur
...
particle_color = ti.Vector.field(3, dtype=ti.f32, shape=N)   # AJOUTÉ
...
def init_particles():
    for i in range(N):
        ...
        hue = i / max(1, N)                       # AJOUTÉ
        particle_color[i] = hsv_to_rgb(hue, 0.85, 1.00)   # AJOUTÉ
...
def draw_particles():
    ...
    for p_idx in range(N):
        ...
        # if hit[p_idx] == 1:
        #     color = HIT_COLOR
        color = particle_color[p_idx]  # MODIFIÉ
        if hit[p_idx] == 1:
            color = 0.35 * color + 0.65 * ti.Vector([1.0, 1.0, 1.0])  # AJOUTÉ
