...
ti.init(arch=ti.gpu)  # MODIFIÉ : vous avez basculé sur le GPU
...
PROFILE = True                     # AJOUTÉ
PROFILE_EVERY = 240                # AJOUTÉ
ENABLE_RENDER = True               # AJOUTÉ
ENABLE_AUDIO = True                # AJOUTÉ
ENABLE_PARTICLE_DRAW = True        # AJOUTÉ
ENABLE_CURVE_DRAW = True           # AJOUTÉ
ENABLE_TEXT_DRAW = True            # AJOUTÉ
...
frame_count = 0                     # AJOUTÉ
t_step_acc = 0.0                    # AJOUTÉ
t_copy_acc = 0.0                    # AJOUTÉ
t_audio_acc = 0.0                   # AJOUTÉ
t_draw_acc = 0.0                    # AJOUTÉ
t_total_acc = 0.0                   # AJOUTÉ
...
while gui.running:
    t0_total = time.perf_counter()  # AJOUTÉ
    ...
    # Simulation
    # --------------------------------------------------------
    t0 = time.perf_counter()        # AJOUTÉ
    for _ in range(SUBSTEPS):
        step()
    ti.sync()                       # AJOUTÉ : PolarAccs la fin réelle du travail GPU avant mesure
    t_step_acc += time.perf_counter() - t0   # AJOUTÉ
    ...
    p = pos.to_numpy()
    h = hit.to_numpy()
    t_copy_acc += time.perf_counter() - t0   # AJOUTÉ
    ...
    t_audio_acc += time.perf_counter() - t0  # AJOUTÉ
    ...
    if ENABLE_RENDER:
        ...
        gui.show()
    t_draw_acc += time.perf_counter() - t0   # AJOUTÉ

    # --------------------------------------------------------
    # Statistiques
    # --------------------------------------------------------
    dt_total = time.perf_counter() - t0_total    # AJOUTÉ
    t_total_acc += dt_total                      # AJOUTÉ
    frame_count += 1                             # AJOUTÉ
