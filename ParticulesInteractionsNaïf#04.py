...
# trail_prim_vertices = ti.Vector.field(2, dtype=ti.f32, shape=2 * N)  # >>> SUPPRIME / REMPLACE
velocities = ti.Vector.field(2, dtype=ti.f32, shape=N)
forces = ti.Vector.field(2, dtype=ti.f32, shape=N)
densities = ti.field(dtype=ti.f32, shape=N)  # >>> MODIF : scalaire plus cohérent
...
# @ti.kernel
# def build_trail_vertices():
#     for i in range(N):
#         trail_prim_vertices[2 * i] = previous_positions[i]
#         trail_prim_vertices[2 * i + 1] = CartPoss[i]
# >>> SUPPRIME / REMPLACE : ancienne traîne à 1 segment
...
def main():
    ...
    while window.running:
        ...
        # >>> AJOUT : mémorisation de l'état courant
        ...
        #     color=(0.6, 0.8, 1.0)
        # )
        # >>> SUPPRIME / REMPLACE : ancienne traîne par segments

        with gui.sub_window("Infos", 0.02, 0.02, 0.32, 0.22):
            gui.text(f"N = {N}")
            gui.text("Mode : interactions completes naives")
            gui.text("Complexite : O(N^2)")
            gui.text(f"TRAIL_LENGTH = {TRAIL_LENGTH}")  # >>> AJOUT
