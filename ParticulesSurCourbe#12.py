...
def draw_energy_history(gui, values, x0=0.02, y0=0.62, w=0.26, h=0.12, color=0x66FFAA):
    ...
    gui.lines(pts[:-1], pts[1:], radius=1.0, color=0x66FFAA)  # MODIFIÉ
...
while gui.running:
    ...
    if ENABLE_RENDER:
        ...
        note_colors_draw_rgb = note_colors_np.copy()  # MODIFIÉ
        note_hit_mask = (note_hit_np == 1)
        if np.any(note_hit_mask):
            note_colors_draw_rgb[note_hit_mask] = (
                    0.35 * note_colors_draw_rgb[note_hit_mask] + 0.65 * 1.0
            )  # MODIFIÉ

        note_colors_draw_hex = rgb_array_to_hex_colors(note_colors_draw_rgb)  # AJOUTÉ
        gui.circles(note_pos_np, radius=PARTICLE_RADIUS, color=note_colors_draw_hex)  # MODIFIÉ

        chord_rgb = hsv_to_rgb_cpu(chord_hue, 0.95, 1.0)
        chord_colors_draw_rgb = np.tile(chord_rgb[None, :], (N_CHORD, 1))  # MODIFIÉ
        chord_hit_mask = (chord_hit_np == 1)
        if np.any(chord_hit_mask):
            chord_colors_draw_rgb[chord_hit_mask] = (
                    0.35 * chord_colors_draw_rgb[chord_hit_mask] + 0.65 * 1.0
            )  # MODIFIÉ

        chord_colors_draw_hex = rgb_array_to_hex_colors(chord_colors_draw_rgb)  # AJOUTÉ
        gui.circles(chord_pos_np, radius=CHORD_PARTICLE_RADIUS, color=chord_colors_draw_hex)  # MODIFIÉ
