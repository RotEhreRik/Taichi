...
W, H = 1000, 700
N_NOTE = 12                                  # MODIFIÉ : ancien N
N_CHORD = 1                                  # AJOUTÉ
DT = 1.0 / 240.0
SUBSTEPS = 2
GRAVITY = 1.2
PARTICLE_RADIUS = 4.0
CHORD_PARTICLE_RADIUS = 6.0                  # AJOUTÉ

# ============================================================
# Paramètres du set "accord"
# ============================================================

CHORD_SPEED_SCALE = 0.35                     # AJOUTÉ
CHORD_GRAVITY_SCALE = 0.45                   # AJOUTÉ
CHORD_COLOR_SPEED = 0.08                     # AJOUTÉ
CHORD_PARTICLES_PLAY_SOUND = False           # AJOUTÉ
...
ALL_CHORD_SCALES = build_all_chord_scales(ROOT_NOTE_HZ, CHORD_LIBRARY, NB_OCTAVES)   # AJOUTÉ
current_chord_index = 0                                                                # AJOUTÉ
current_chord_name, NOTES_HZ = ALL_CHORD_SCALES[current_chord_index]                  # AJOUTÉ
...
note_pos = ti.Vector.field(2, dtype=ti.f32, shape=N_NOTE)            # AJOUTÉ
note_old_pos = ti.Vector.field(2, dtype=ti.f32, shape=N_NOTE)        # AJOUTÉ
note_vel = ti.Vector.field(2, dtype=ti.f32, shape=N_NOTE)            # AJOUTÉ

note_hit = ti.field(dtype=ti.i32, shape=N_NOTE)                      # AJOUTÉ
note_impact_event = ti.field(dtype=ti.i32, shape=N_NOTE)             # AJOUTÉ
note_particle_index = ti.field(dtype=ti.i32, shape=N_NOTE)           # AJOUTÉ
note_particle_color = ti.Vector.field(3, dtype=ti.f32, shape=N_NOTE) # AJOUTÉ

# ============================================================
# Champs Taichi : set accords
# ============================================================

chord_pos = ti.Vector.field(2, dtype=ti.f32, shape=N_CHORD)          # AJOUTÉ
chord_old_pos = ti.Vector.field(2, dtype=ti.f32, shape=N_CHORD)      # AJOUTÉ
chord_vel = ti.Vector.field(2, dtype=ti.f32, shape=N_CHORD)          # AJOUTÉ

chord_hit = ti.field(dtype=ti.i32, shape=N_CHORD)                    # AJOUTÉ
chord_impact_event = ti.field(dtype=ti.i32, shape=N_CHORD)           # AJOUTÉ
chord_change_event = ti.field(dtype=ti.i32, shape=())                # AJOUTÉ
...
CURVE2_COLOR = ti.Vector([255 / 255, 180 / 255, 90 / 255])           # AJOUTÉ
...
@ti.kernel
def draw_chord_particles(chord_hue: ti.f32):                    # AJOUTÉ
...
start_time = time.perf_counter()                                  # AJOUTÉ
