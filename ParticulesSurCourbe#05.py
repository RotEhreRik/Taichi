...
old_pos = ti.Vector.field(2, dtype=ti.f32, shape=N)   # AJOUTÉ
...
def step():
    for i in range(N):
        ...
        # MODIFIÉ : détection par franchissement
        old_yc = curve_y(old_pos[i].x)   # AJOUTÉ
        new_yc = curve_y(pos[i].x)       # AJOUTÉ

        crossed = (old_pos[i].y > old_yc) and (pos[i].y <= new_yc)  # AJOUTÉ

        if crossed:  # MODIFIÉ
            n = curve_normal(pos[i].x)

            # Reprojection simple au-dessus de la courbe
            pos[i].y = new_yc + 1e-4  # MODIFIÉ
            ...
            hit[i] = 1  # MODIFIÉ : le son correspond au franchissement

        # AJOUTÉ : sécurité si pénétration résiduelle sans nouveau son
        elif pos[i].y <= new_yc:
            pos[i].y = new_yc + 1e-4
...
def play_note(note_index):
    ...
    ch = pygame.mixer.find_channel()  # AJOUTÉ
    if ch is not None:
        ch.play(sound)
...
while gui.running:
    ...
    # --------------------------------------------------------
    # Copies GPU -> CPU (gardées pour l'audio)
    # --------------------------------------------------------
    t0 = time.perf_counter()
    h = hit.to_numpy()
    p = pos.to_numpy()
