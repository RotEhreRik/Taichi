...
N = 64 # taille logique de la grille
...
def render():
    ...
    for i, j in current:
        # >>> LIGNE MODIFIÉE : on N'affiche en blanc que les cellules vivantes
        if current[i, j] != 0:
            pixels[i, j] = 1.0
...
def main():
    ...
    gui = ti.GUI("Jeu de la vie sparse GPU - Taichi", res=(WINDOW_W, WINDOW_H))
    clear_all()
