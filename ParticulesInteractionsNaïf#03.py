...
previous_positions = ti.Vector.field(2, dtype=ti.f32, shape=N)  # >>> AJOUT
trail_vertices = ti.Vector.field(2, dtype=ti.f32, shape=2 * N)  # >>> AJOUT
...
def build_trail_vertices():
    for i in range(N):
        trail_vertices[2 * i] = previous_positions[i]
        trail_vertices[2 * i + 1] = positions[i]
