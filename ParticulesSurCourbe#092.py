...
W, H = 700, 500
N_NOTE = 12
N_CHORD = 2                               # MODIFIÉ : 2 particules secondaires
DT = 1.0 / 240.0
SUBSTEPS = 2
GRAVITY = 1.2
PARTICLE_RADIUS = 4.0
...
ROOT_SELECTOR_NAME = "root_selector"      # AJOUTÉ
ROOT_SELECTOR_INTERVALS = [0, 2, 7, 12]   # AJOUTÉ : accord générateur des fondamentales
...
def build_root_candidates(root_hz, intervals):
    # AJOUTÉ : liste des fondamentales candidates
    roots = []
    for interval in intervals:
        freq = root_hz * (2.0 ** (interval / 12.0))
        roots.append(freq)
    return roots

ROOT_NOTE_CANDIDATES = build_root_candidates(ROOT_NOTE_HZ, ROOT_SELECTOR_INTERVALS)  # AJOUTÉ
current_root_index = 0                                                                # AJOUTÉ
...
chord_change_event = ti.field(dtype=ti.i32, shape=())   # particule 0 -> type d'accord
root_change_event = ti.field(dtype=ti.i32, shape=())    # AJOUTÉ : particule 1 -> fondamentale
...
def init_chord_particles():
    for i in range(N_CHORD):
        # MODIFIÉ : placement explicite des 2 particules secondaires
        chord_pos[i] = ti.Vector([0.18 + 0.20 * i, 0.92])
    ...
    root_change_event[None] = 0                 # AJOUTÉ
...
def step_chord_particles():
    for i in range(N_CHORD):
        ...
        if crossed:
            ...
            if i == 0:
                chord_change_event[None] = 1       # MODIFIÉ : particule 0 = type d'accord
            else:
                root_change_event[None] = 1        # AJOUTÉ : particule 1 = fondamentale
...
def draw_chord_particles(chord_hue: ti.f32):
    ...
    for p_idx in range(N_CHORD):
        ...
        # MODIFIÉ : teinte différente par particule secondaire
        hue = chord_hue + 0.18 * p_idx
        color = hsv_to_rgb(hue, 0.95, 1.00)
...
def apply_musical_state(chord_index, root_index):
    # AJOUTÉ : remplace apply_chord(...) par une version à 2 degrés de liberté
    global current_chord_index, current_root_index
    global current_chord_name, NOTES_HZ, NOTE_COUNT, tone_bank

    current_chord_index = chord_index % len(CHORD_LIBRARY)
    current_root_index = root_index % len(ROOT_NOTE_CANDIDATES)

    chord_name, chord_intervals = CHORD_LIBRARY[current_chord_index]
    selected_root_hz = ROOT_NOTE_CANDIDATES[current_root_index]

    current_chord_name = f"{chord_name} @ {selected_root_hz:.2f} Hz"
    NOTES_HZ = build_chord_scale(selected_root_hz, chord_intervals, NB_OCTAVES)
    NOTE_COUNT = len(NOTES_HZ)
...
apply_musical_state(0, 0)                    # MODIFIÉ
...
while gui.running:
    ...
    root_evt = root_change_event[None]            # AJOUTÉ
    t_copy_acc += time.perf_counter() - t0

    # --------------------------------------------------------
    # Changement d'accord / fondamentale
    # --------------------------------------------------------
    if chord_evt == 1:
        apply_musical_state(current_chord_index + 1, current_root_index)   # MODIFIÉ
        chord_change_event[None] = 0

    if root_evt == 1:
        apply_musical_state(current_chord_index, current_root_index + 1)    # AJOUTÉ
        root_change_event[None] = 0
    ...
    if PROFILE and frame_count % PROFILE_EVERY == 0:
        ...
        print(f"Root index : {current_root_index} / {len(ROOT_NOTE_CANDIDATES)}")
