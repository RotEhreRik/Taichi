...
# Réglage pour avoir une quantité de mouvement totale initiale nulle
...
init_pos = ti.Vector.field(2, dtype=ti.f64, shape=N)
init_vel = ti.Vector.field(2, dtype=ti.f64, shape=N)
...
# Copie des données NumPy vers des champs Taichi lisibles dans les kernels
for i in range(N):
    masses[i] = masses_np[i]
    init_pos[i] = ti.Vector([init_pos_np[i, 0], init_pos_np[i, 1]])
    init_vel[i] = ti.Vector([init_vel_np[i, 0], init_vel_np[i, 1]])
...
def initialize():
    for method, i in ti.ndrange(METHODS, N):
        pos[method, i] = init_pos[i]
        vel[method, i] = init_vel[i]
...
@ti.kernel
def verlet_kick(old_acc_arr: ti.types.ndarray(), dt: ti.f64):  # <<< MODIFIÉ
    for i in range(N):
        old_a = ti.Vector([old_acc_arr[i, 0], old_acc_arr[i, 1]])  # <<< MODIFIÉ
...
for step in range(1, STEPS + 1):
    ...
    acc_buf = np.ascontiguousarray(acc_buf)  # <<< AJOUT
    verlet_drift(DT)
    compute_acc_for_method(1)
    verlet_kick(acc_buf, DT)  # <<< MODIFIÉ
...
print("Image : output/three_body_taichi_compare.png")
print("CSV   : output/three_body_taichi_compare.csv")